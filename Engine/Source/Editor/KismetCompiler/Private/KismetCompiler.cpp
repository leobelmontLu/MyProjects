...
void FKismetCompilerContext::ExpandTunnelsAndMacros(UEdGraph* SourceGraph)
{
	...
	for (TArray<UEdGraphNode*>::TIterator NodeIt(SourceGraph->Nodes); NodeIt; ++NodeIt)
	{
		...
		if (UK2Node_MacroInstance* MacroInstanceNode = Cast<UK2Node_MacroInstance>(*NodeIt))
		{
			...
			{
				//Changed by swd for map begin 2019年1月30日12:42:58
						/*
				...
				const TCHAR* ValuePrefix = TEXT("Value");
				const FEdGraphPinType& WildcardType = MacroInstanceNode->ResolvedWildcardType;
				bool bIsMap = WildcardType.IsMap();
				for (UEdGraphNode* const ClonedNode : ClonedGraph->Nodes)
				{
					if (ClonedNode)
					{
						bool IsAssignNode = ClonedNode->IsA<UK2Node_AssignmentStatement>();
						for (UEdGraphPin* const ClonedPin : ClonedNode->Pins)
						{
							if (ClonedPin && (ClonedPin->PinType.PinCategory == UEdGraphSchema_K2::PC_Wildcard))
							{
								FEdGraphPinType& ClonePinType = ClonedPin->PinType;
								if (bIsMap)
								{
									if (ClonePinType.IsMap())
									{
										ClonePinType.PinCategory = WildcardType.PinCategory;
										ClonePinType.PinSubCategory = WildcardType.PinSubCategory;
										ClonePinType.PinSubCategoryObject = WildcardType.PinSubCategoryObject;
										ClonePinType.PinValueType.TerminalCategory = WildcardType.PinValueType.TerminalCategory;
										ClonePinType.PinValueType.TerminalSubCategory = WildcardType.PinValueType.TerminalSubCategory;
										ClonePinType.PinValueType.TerminalSubCategoryObject = WildcardType.PinValueType.TerminalSubCategoryObject;
									}
									else if (ClonedPin->PinName.ToString().StartsWith(ValuePrefix))
									{
										if (IsAssignNode && ClonedPin->LinkedTo.Num() > 0 && ClonedPin->LinkedTo[0]->PinType.PinCategory != UEdGraphSchema_K2::PC_Wildcard)
										{
											const FEdGraphPinType& LinkToType = ClonedPin->LinkedTo[0]->PinType;
											ClonePinType.PinCategory = LinkToType.PinCategory;
											ClonePinType.PinSubCategory = LinkToType.PinSubCategory;
											ClonePinType.PinSubCategoryObject = LinkToType.PinSubCategoryObject;
										}
										else
										{
											ClonePinType.PinCategory = WildcardType.PinValueType.TerminalCategory;
											ClonePinType.PinSubCategory = WildcardType.PinValueType.TerminalSubCategory;
											ClonePinType.PinSubCategoryObject = WildcardType.PinValueType.TerminalSubCategoryObject;
										}
									}
									else
									{
				...
				//Changed by swd for map end 2019年1月30日12:42:58
			}
